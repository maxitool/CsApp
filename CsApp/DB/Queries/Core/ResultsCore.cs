    ...
    public class ResultsCore : ParentCore
    {
        ...
        public static string Insert(Models.Results result)
        {
            return $@"INSERT INTO Results 
                ...
                VALUES ({result.id_file}, '{result.delta_date.ToString()}', '{result.min_date.ToString()}', {result.avg_execution_time}, {result.avg_value}, 
                {result.median_value}, {result.max_value}, {result.min_value}) 
                RETURNING id";
        }
